...
class RegisterForm(FlaskForm):
    ...
    def validate_username(self, username):
        conn = connect_db()
        existing_user_username = conn.execute(
            "SELECT * FROM user WHERE username = ?", (username.data,)
        ).fetchone()
        conn.commit()
        conn.close()
...
class UpdateForm(FlaskForm):
    ...
    oldpassword = PasswordField(
        validators=[InputRequired(), Length(min=8, max=20)],
        render_kw={"placeholder": "Previous Password"},
    )
...
def register():
    ...
    if form.validate_on_submit():
        ...
        conn.execute(
            "INSERT INTO user (email, username, roll_number, pass_word) VALUES (?, ?, ?, ?)",
            (
                form.email.data,
                form.username.data,
                form.rollnumber.data,
                hashed_password,
            ),
        )
        conn.commit()
    ...
    if form.validate_on_submit():
        ...
        if user:
            if bcrypt.check_password_hash(user[4], form.oldpassword.data):
                ...
                conn.execute(
                    "UPDATE user set pass_word = ? where username = ?",
...
def predictSpecies():
    if request.method == "POST":
        ...
        if dataset == "spiral":
            m = "/Users/apple/Desktop/Final_Code/parkinson.pkl"
        else:
            m = "/Users/apple/Desktop/Final_Code/parkinson_wave.pkl"
        model = joblib.load(m)
